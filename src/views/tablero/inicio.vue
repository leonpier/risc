                ...
                <v-card>
                    <v-form ref="form" lazy-validation>
                        <v-row no-gutters style="height: 270px;">
                            ...
                            <v-col md="4" style="height: 10px;">
                                <v-card-text class="pa-2" >
                                    <v-autocomplete :items="establecimientos" v-model="search.establecimiento"  label="IPRESS" item-text="name" item-value="id" multiple dense outlined ></v-autocomplete>
                                </v-card-text>
                            </v-col>
                            <v-col md="4" style="height: 10px;">
                                <v-card-text class="pa-2" >
                                    <v-autocomplete :items="estrategias" v-model="search.estrategia"  label="Estratégia" item-text="descripcion" item-value="idestrategia" dense outlined ></v-autocomplete>
                                </v-card-text>
                            </v-col>
                        ...
                        <template v-slot:default>
                              ...
                              <tbody>
                                <tr v-for="item in tabla" :key="item.name">
                                  ...
                                  <td class="text-center">{{ parseInt(item.porcentaje ) }} %</td>
                                  <td style="text-align:center">
                                    <span style="font-size:28px" :class="semaforo(parseFloat(parseInt(item.porcentaje)/100))"></span>
                                  </td>
                                </tr>
                              </tbody>
                        </template>
...
export default {
  ...
  methods: {
    ...
    async validate () {
        await this.axios.get(this.$api_url + '/tablero/inicio',{
                params: {
    ...
    async getEstrategias() {
        await this.axios.get(this.$api_url + '/filtros/estrategias').then(response => {
